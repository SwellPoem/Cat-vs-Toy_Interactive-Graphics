...
export default function Sitting_Luna(){
  ...
  this.threeGroup = new THREE.Group();
    ...
    this.torso.geometry.applyMatrix4(new THREE.Matrix4().makeRotationY(Math.PI/3));
    this.torso.geometry.applyMatrix4(new THREE.Matrix4().makeTranslation(0,-this.bodyHeight/2,0));
    
    // chest
  
    var chestGeom = new THREE.CylinderGeometry(5,19,65,3);
    this.chest = new THREE.Mesh(chestGeom, goldMat);
    //this.chest.rotation.y = Math.PI/2;
    this.chest.geometry.applyMatrix4(new THREE.Matrix4().makeRotationY(Math.PI/3));
    this.chest.position.set(0,-57,-7);
    ...
    // // knees
    var kneeGeom = new THREE.BoxGeometry(10,32,27);
    kneeGeom.applyMatrix4(new THREE.Matrix4().makeTranslation(0,17,0));
    this.rightKnee = new THREE.Mesh(kneeGeom, brownMat);
    this.rightKnee.rotation.y = -Math.PI/8;
    ...
    // // legs = back legs
    var legGeom = new THREE.BoxGeometry(12,6,7);
    ...
    stripe2.position.x = 15;
    stripe2.position.y = 7;
...
Sitting_Luna.prototype.updateTail = function(t){
...
Sitting_Luna.prototype.interactWithBall = function(ballPos){
...
Sitting_Luna.prototype.lookAt = function(v){
...
Sitting_Luna.prototype.replaceFoot = function(side){
    _this = this;
    var angles =  {theta:0, alpha:0, beta:0};
    this.updateArm(side, angles, 2, "strong.easeInOut", function (){ _this.isFootReplacing = true; });
    _this.footUsed = (_this.footUsed == "right") ? "left" : "right";
}
  
Sitting_Luna.prototype.updateArm = function(side, angles, speed, ease, callBack){
...
Sitting_Luna.prototype.blink = function(){
    _this = this;
...
Sitting_Luna.prototype.moveMouth = function(){
    _this = this;
